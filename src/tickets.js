...
const TYPES = {
  support: { label: 'Support', emoji: '🛠️', description: 'Get help with something' },
  report: { label: 'Report', emoji: '🚨', description: 'Report a player or problem' },
  staff: { label: 'Staff Application', emoji: '👮', description: 'Apply for staff' },
  partnership: { label: 'Partnership', emoji: '🤝', description: 'Partnership requests' }
};
...
function getOpenTicket(guildId, userId) {
  return db
    .prepare('SELECT * FROM tickets WHERE guild_id = ? AND user_id = ? AND closed = 0 LIMIT 1')
    .get(guildId, userId);
}
...
function updateTicket(channelId, data) {
  ...
  db.prepare(`UPDATE tickets SET ${set} WHERE channel_id = ?`).run(...values, channelId);
}
...
function safeChannelName(type, username) {
  const cleanUser = String(username).toLowerCase().replace(/[^a-z0-9-]/g, '').slice(0, 55);
...
function ticketButtons(ticket) {
  ...
  if (ticket.closed) {
    row.addComponents(
      new ButtonBuilder().setCustomId('ticket:reopen').setLabel('Reopen Ticket').setEmoji('🔓').setStyle(ButtonStyle.Success),
    ...
    row.addComponents(
      new ButtonBuilder().setCustomId('ticket:claim').setLabel(ticket.claimed_by ? 'Claimed' : 'Claim Ticket').setEmoji('🙋').setStyle(ticket.claimed_by ? ButtonStyle.Secondary : ButtonStyle.Primary).setDisabled(Boolean(ticket.claimed_by)),
      new ButtonBuilder().setCustomId('ticket:close').setLabel('Close Ticket').setEmoji('🔒').setStyle(ButtonStyle.Danger),
      new ButtonBuilder().setCustomId('ticket:transcript').setLabel('Transcript').setEmoji('📜').setStyle(ButtonStyle.Secondary),
      new ButtonBuilder().setCustomId('ticket:add').setLabel('Add Member').setEmoji('➕').setStyle(ButtonStyle.Secondary)
    );
  }
  return row;
}
...
async function createTicket(interaction, type) {
  ...
  try {
    ...
    const channel = await interaction.guild.channels.create({
      ...
      topic: `${config.label} ticket • Owner: ${interaction.user.id}`,
      permissionOverwrites: [
        ...
        },
        {
          ...
          allow: [PermissionFlagsBits.ViewChannel, PermissionFlagsBits.SendMessages, PermissionFlagsBits.ReadMessageHistory, PermissionFlagsBits.ManageChannels, PermissionFlagsBits.ManageMessages, PermissionFlagsBits.AttachFiles]
        }
      ]
    });
    ...
    const message = '❌ I could not create your ticket. Please contact staff.';
...
async function claimTicket(interaction) {
  try {
    ...
    if (ticket.closed) return interaction.reply({ content: '❌ This ticket is closed.', ephemeral: true });
    if (ticket.claimed_by) return interaction.reply({ content: '❌ This ticket has already been claimed.', ephemeral: true });

    updateTicket(interaction.channel.id, { claimed_by: interaction.user.id });
    const updated = getTicket(interaction.channel.id);

    await interaction.channel.send({
      embeds: [new EmbedBuilder().setDescription(`🙋 **${interaction.user}** claimed this ticket.`).setColor(0x57f287).setTimestamp()]
    });

    await interaction.message.edit({ components: [ticketButtons(updated)] });
...
async function closeTicket(interaction) {
  try {
    ...
    const allowed = ticket.user_id === interaction.user.id || isStaff(interaction);
    if (!allowed) return interaction.reply({ content: '❌ Only the ticket owner or staff can close this ticket.', ephemeral: true });
    ...
    await interaction.channel.setName(`closed-${interaction.channel.name}`.replace(/^closed-/, '').slice(0, 90));

    const updated = getTicket(interaction.channel.id);
    await interaction.message.edit({ components: [ticketButtons(updated)] });

    await interaction.channel.send({
      embeds: [new EmbedBuilder().setTitle('🔒 Ticket Closed').setDescription(`This ticket was closed by ${interaction.user}.\n\nStaff can reopen or delete it.`).setColor(0xed4245).setTimestamp()]
    });
...
async function reopenTicket(interaction) {
  try {
    if (!isStaff(interaction)) {
      return interaction.reply({ content: '❌ Only staff members can reopen tickets.', ephemeral: true });
    }

    const ticket = getTicket(interaction.channel.id);
    if (!ticket) return interaction.reply({ content: '❌ This is not a ticket channel.', ephemeral: true });
    if (!ticket.closed) return interaction.reply({ content: '❌ This ticket is already open.', ephemeral: true });
    ...
    await interaction.channel.setName(interaction.channel.name.replace(/^closed-/, '').slice(0, 90));

    const updated = getTicket(interaction.channel.id);
    await interaction.message.edit({ components: [ticketButtons(updated)] });

    await interaction.channel.send({
      embeds: [new EmbedBuilder().setTitle('🔓 Ticket Reopened').setDescription(`This ticket was reopened by ${interaction.user}.`).setColor(0x57f287).setTimestamp()]
    });
...
async function transcript(interaction) {
  try {
    if (!isStaff(interaction)) {
      return interaction.reply({ content: '❌ Only staff members can create transcripts.', ephemeral: true });
    }
    ...
    for (const message of sorted) {
      ...
      const content = message.content || '[Attachment/Embed/No text]';
      lines.push(`[${time}] ${message.author.tag}: ${content}`);
    }

    const buffer = Buffer.from(lines.join('\n'), 'utf8');
    const file = new AttachmentBuilder(buffer, { name: `${interaction.channel.name}-transcript.txt` });

    return interaction.editReply({ content: '📜 Transcript generated.', files: [file] });
...
async function deleteTicket(interaction) {
  try {
    ...
    setTimeout(() => {
      interaction.channel.delete('Ticket deleted').catch(error => console.error('DELETE CHANNEL ERROR:', error));
    }, 1500);
...
async function addMember(interaction) {
  try {
    ...
    const modal = new ModalBuilder().setCustomId('ticket:add_modal').setTitle('Add Member');
...
async function addMemberModal(interaction) {
  try {
    ...
    const value = interaction.fields.getTextInputValue('member').trim().replace(/[<@!>]/g, '');

    if (!/^\d{15,25}$/.test(value)) {
      return interaction.reply({ content: '❌ Please enter a valid Discord user ID or mention.', ephemeral: true });
    }

    const member = await interaction.guild.members.fetch(value).catch(() => null);
    if (!member) return interaction.reply({ content: `❌ Failed to find that member.`, ephemeral: true });
    ...
    await interaction.channel.send({
      embeds: [new EmbedBuilder().setDescription(`➕ ${member} was added to this ticket by ${interaction.user}.`).setColor(0x57f287).setTimestamp()]
    });

    return interaction.reply({ content: `✅ Added ${member} to this ticket.`, ephemeral: true });

  } catch (error) {
    console.error('ADD MEMBER MODAL ERROR:', error);
    if (!interaction.replied && !interaction.deferred) {
      await interaction.reply({ content: '❌ Failed to add that member.', ephemeral: true }).catch(() => {});
    }
  }
}
...
async function handleButton(interaction) {
  switch (interaction.customId) {
    ...
    case 'ticket:close': return closeTicket(interaction);
    case 'ticket:reopen': return reopenTicket(interaction);
    case 'ticket:transcript': return transcript(interaction);
    case 'ticket:delete': return deleteTicket(interaction);
    case 'ticket:add': return addMember(interaction);
    default: return;
  }
}

async function handleModal(interaction) {
  if (interaction.customId === 'ticket:add_modal') {
    return addMemberModal(interaction);
  }
}
